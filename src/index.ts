...
export const raindropFetch = async <T>(
  path: string,
  options: RequestInit = {},
): Promise<T | null> => {
  try {
    ...
    console.error(`Raindrop fetch failed: ${path}`, e)
    return null
  }
}
...
export const searchRaindrops = async ({
  collection_id = 0,
  search,
  sort,
  page = 0,
  ...
  notag?: boolean
  domain?: string
  media?: "link" | "article" | "image" | "video" | "document" | "audio"
...
export const getRaindrop = async ({ id }: { id: number }) => {
...
export const createRaindrop = async ({
  link,
  title,
  excerpt,
  note,
...
export const updateRaindrop = async ({
  id,
  title,
  excerpt,
  note,
...
export const deleteRaindrop = async ({ id }: { id: number }) => {
...
export const createRaindrops = async ({
  items,
}: {
  items: Array<{
    link: string
  ...
  const body = items.map(
    ({ link, title, excerpt, note, tags, important, collection_id }) => {
      const item: Record<string, unknown> = { link, pleaseParse: {} }
      if (title) item.title = title
      if (excerpt) item.excerpt = excerpt
      if (note) item.note = note
      if (tags) item.tags = tags
      if (important != null) item.important = important
      if (collection_id != null) item.collection = { $id: collection_id }
      return item
    },
  )

  const data = await raindropFetch<{ result: boolean; items: unknown[] }>(
    "/raindrops",
    {
      method: "POST",
  ...
  if (!data) return err("failed to create raindrops")
...
server.registerTool(
  "create_raindrops",
  {
    description: "Create multiple bookmarks in bulk",
    inputSchema: {
      items: z
        .array(
          z.object({
        ...
        .describe("Array of bookmarks to create"),
    },
  },
  createRaindrops,
)

export const bulkRaindrops = async ({
  operation,
  collection_id,
  ids,
  to_collection_id,
...
server.registerTool(
  "get_collections",
  { description: "List all root collections", inputSchema: {} },
  getCollections,
)

export const getChildCollections = async () => {
...
server.registerTool(
  ...
  { description: "List all child (nested) collections", inputSchema: {} },
  getChildCollections,
)
...
export const cleanupCollections = async ({ confirm }: { confirm: boolean }) => {
  if (!confirm)
    return ok({
...
export const getHighlights = async ({
  ...
  const path =
    raindrop_id != null
      ...
      : collection_id != null
        ? `/highlights/${collection_id}`
        : "/highlights"
  const data = await raindropFetch<{ items: unknown[] }>(path)
  if (!data) return err("failed to fetch highlights")
  return ok(data.items)
}
...
server.registerTool(
  ...
  {
    description:
      "Get highlights — all in library (no args), by collection, or by bookmark",
    inputSchema: {
      raindrop_id: z
        .number()
        .optional()
        .describe("Get highlights for a specific bookmark"),
      collection_id: z
        .number()
        .optional()
        .describe("Get highlights in a specific collection"),
    },
  },
  getHighlights,
)

export const manageHighlight = async ({
  operation,
  raindrop_id,
  highlight_id,
  text,
...
server.registerTool(
  "get_user",
  { description: "Get authenticated user info and stats", inputSchema: {} },
  getUser,
)
...
export const checkUrlsExist = async ({ urls }: { urls: string[] }) => {
...
export const libraryAudit = async ({
  include_items,
}: {
...
export const emptyTrash = async ({ confirm }: { confirm: boolean }) => {
  if (!confirm)
    return ok({ message: "Pass confirm: true to permanently empty the trash." })
